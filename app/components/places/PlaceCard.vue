<template>
  <v-card class="place-card" elevation="1" rounded="lg" hover>
    <v-btn
      icon
      size="small"
      variant="text"
      class="bookmark-btn"
      :aria-label="isBookmarked ? `Remove ${place.name} from saved` : `Save ${place.name}`"
    ...
    <NuxtLink :to="`/places/${place.id}`" class="card-link">
      <v-card-text class="pt-4">
        <div class="arabic-name">{{ place.arabicName }}</div>

        <div class="d-flex align-center flex-wrap ga-2 mt-1">
          <div class="display-name">{{ place.name }}</div>
          <v-chip size="x-small" variant="tonal" color="primary" class="type-chip">
            {{ typeLabel }}
          </v-chip>
        </div>

        <p class="short-description mt-2">{{ place.shortDescription }}</p>
        ...
        <div v-if="place.themes?.length" class="themes mt-3">
          <v-chip
            v-for="theme in place.themes.slice(0, 4)"
            :key="theme"
            size="x-small"
            variant="text"
            class="theme-chip"
...
const bookmarkKey = computed(() => `place:${props.place.id}`);
const isBookmarked = computed(() => has(bookmarkKey.value));
const toggleBookmark = () => toggle(bookmarkKey.value);

const typeLabel = computed(
  () => PLACE_TYPE_FILTERS.find((t) => t.value === props.place.placeType)?.label ?? props.place.placeType
);
</script>
