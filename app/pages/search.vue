<template>
  <v-container>
    ...
    <v-text-field
      v-model="query"
      label="Search Quran (translations only)"
    ...
    <!-- Empty -->
    <v-alert
      v-if="!pending && query && !results.length"
      type="info"
      variant="tonal"
      class="mt-4"
    >
...
const query = ref("");
const surah = ref("all");
const edition = ref("en.pickthall");
...
/* ------------------ navigation ------------------ */
const gotoAyah = (ayah) => {
  router.push(`/surah/${ayah.surah.number}#ayah-${ayah.numberInSurah}`);
};
</script>
