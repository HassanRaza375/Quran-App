<template>
    <v-container>
        ...
        <v-row>
            <v-col cols="12">
                <v-text-field v-model="searchText" append-inner-icon="mdi-magnify" label="Search Surah"
            ...
            <v-col v-for="surah in filteredSurahs" :key="surah.surahNo" cols="12" sm="6" md="4" lg="3" xl="2">
                ...
                <v-lazy :min-height="137" :options="{ 'threshold': 0.5 }" transition="fade-transition">
                    <v-card class="surah-card" elevation="1" rounded="lg" hover>
                        <!-- Favorite button -->
                        <v-btn icon size="small" variant="text" class="fav-btn"
                            @click.stop="toggleFav(surah, surah.surahNo)">
